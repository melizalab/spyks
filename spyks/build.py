...
def build_module(cppfile, name, path, **kwargs):
    ...
    command = ["c++", "-O3", "-Wall", "-shared", "-std=c++11", "-fPIC",]
...
def compile_script(argv=None):
    ...
    p.add_argument("--optimize", help="enable aggressive compiler optimizations", action="store_true")
    ...
    if not args.skip_compile:
        build_module(cppfile, model["name"], path, version=model["version"], optimize=args.optimize)
