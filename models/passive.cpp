...
struct passive {
        ...
        passive (value_type const * p, interpolator_type f)
             : p(p), forcing(f) {}
...
PYBIND11_MODULE(passive, m) {
        typedef double value_type;
        typedef double time_type;
        typedef spyks::nn_interpolator<value_type, time_type> interpolator;
        typedef spyks::passive<value_type, interpolator> model;
        m.doc() = "biophysical neuron model with leak only";
        m.attr("name") = py::cast("passive");
