...
struct nn_interpolator {
        ...
        value_type operator()(time_type t, size_t j) const {
                return data.data(index(t), j);
        }
        size_t index(time_type t) const {
                if (t < 0) return 0;
...
template <typename value_type, typename interpolator_type, typename time_type=double>
struct adex {
...
PYBIND11_PLUGIN(adex) {
        ...
        typedef double time_type;
        typedef nn_interpolator<value_type, time_type> interpolator;
        typedef adex<value_type, interpolator, time_type> model;

        py::module m("adex", "adaptive exponential integrate and fire model");
        py::class_<interpolator>(m, "interpolator")
                .def(py::init<py::array, double>())
                .def("__call__", [](interpolator const & d, time_type t, size_t j) {
                                return d(t, j);
                        });
