...
const double x4 = exp((-X[0] + p[20])/p[21]) + 1;
                dXdt[0] = (Iinj + x1*pow(X[3], 4)*p[6] + x1*X[4]*p[7] + (x0 + p[1])*p[4] + (x0 + p[2])*pow(X[1], 3)*X[2]*p[5])/p[0];
dXdt[1] = (1.0/2.0)*x3*(tanh((X[0] - p[8])/p[9]) - 2*X[1] + 1)/(x3*p[10] + 1);
dXdt[2] = (-x4*(X[2] - 1)*exp((-X[0] + p[17])/p[18])*p[16] - X[2]*p[19])/x4;
dXdt[3] = ((1.0/2.0)*tanh((X[0] - p[22])/p[23]) - X[3] + 1.0/2.0)/(p[24] + p[25]/pow(cosh((X[0] - p[26])/p[27]), 2));
dXdt[4] = ((1.0/2.0)*tanh((X[0] - p[28])/p[29]) - X[4] + 1.0/2.0)/(p[30] + p[31]/pow(cosh((X[0] - p[32])/p[33]), 2));
        }
};
